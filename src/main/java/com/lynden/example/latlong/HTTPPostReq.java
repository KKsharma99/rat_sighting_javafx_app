...
public class HTTPPostReq {
   ...
   public static String getUsername() {
    ...
    public static boolean addSighting(RatSighting sighting) {
    	String command = "{\"token\": \""+getToken()+"\",\"Location_Type\":\""+sighting.getLoc_type()+"\", \"Incident_Zip\":\""+sighting.getZip()+"\",\"Incident_Address\":\"" + sighting.getAddress() + "\",\"City\":\"" + sighting.getCity() + "\",\"Borough\":\"" + sighting.getBorough() + "\",\"Latitude\":\"" + sighting.getLatitude() + "\",\"Longitude\":\"" + sighting.getLongitude() + "\"}";
        String resp = HTTPPostReq.sendPost("https://desolate-taiga-94108.herokuapp.com/api/addRat", command);
    ...
    public static ArrayList<RatSighting> getSightings(int limit, int offset) {
        ...
        try {
            ...
            if (jsonArray != null) {
                ...
                for (int i=0;i<len;i++){
                    JSONObject ratSighting = jsonArray.getJSONObject(i);
                    list.add(new RatSighting(ratSighting.get("Unique_Key").toString(), ratSighting.getString("Created_Date"), ratSighting.getString("Location_Type"), ratSighting.get("Incident_Zip").toString(), ratSighting.getString("Incident_Address"), ratSighting.getString("City"), ratSighting.getString("Borough"), ratSighting.get("Latitude").toString(),ratSighting.get("Longitude").toString()));
                }
            }
        } catch (Exception e) {
            System.out.println(e.toString());
    ...
    public static int login(String login_name, String password) {
        ...
        if(msg.equalsIgnoreCase("login successful")) {
            ...
            return 1;
        } else if(msg.equalsIgnoreCase("Account is locked")) {
        	return 3;
        } else {
    ...
    public static ArrayList<Integer> countMonth() {
        String resp = HTTPPostReq.sendPost("https://desolate-taiga-94108.herokuapp.com/api/stats/countMonth", "{\"token\": \""+getToken()+"\"}");
        ArrayList<Integer> stats = new ArrayList<>();
        try {
            JSONObject json = new JSONObject(resp);
            stats.add(json.getInt("0"));
            stats.add(json.getInt("1"));
            stats.add(json.getInt("2"));
            stats.add(json.getInt("3"));
            stats.add(json.getInt("4"));
            stats.add(json.getInt("5"));
            stats.add(json.getInt("6"));
            stats.add(json.getInt("7"));
            stats.add(json.getInt("8"));
            stats.add(json.getInt("9"));
            stats.add(json.getInt("10"));
            stats.add(json.getInt("11"));
        } catch (Exception e) {
            System.out.println(e.toString());
        }
        return stats;
